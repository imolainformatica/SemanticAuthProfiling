...
class SemanticAuthProfiling
{
    ...
    public static function onBeforePageDisplay(OutputPage &$out, Skin &$skin)
    {
        ...
        $titleObject = Title::newFromText($wgRequest->getText('title'));
        $titleCustom = $titleObject->getText();
        $categorieView = getCategories($titleCustom);
        $title = $titleObject->getPartialURL();
        ...
        if (empty($sysopExists) && empty($masterGroupExists)) {
            ...
            if (count($titleArray) > 1 && (strpos($wgTitle, str_ireplace(" ", "_", "Special:FormEdit")) !== false)) {
                ...
                checkWildcard($out, $wgSelectiveActionCreateCategories[$gruppopPerCategoriaCreate]["*"], $gruppopPerCategoriaCreate);
            }
 ...
 *            gruppopPerCategoriaDelete
 */
function foundGroupsAndDoAction($outputMethod, $currentUserGroups, $wgSelectiveActionCategories)
{
    $gruppopPerCategoria = '';
    foreach ($currentUserGroups as $chiave => $valore) {
